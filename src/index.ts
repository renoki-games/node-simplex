 ...
 *
 * It got forked from https://github.com/joshforisha/fast-simplex-noise-js
 */
...
export class Simplex {
    ...
    raw2D(x: number, y: number): number {
        ...
        const n0 = t0 < 0 ? 0.0 : Math.pow(t0, 4) * this.dot(Simplex.GRAD3D[gi0], [x0, y0]);
        const t1 = 0.5 - x1 * x1 - y1 * y1;
        const n1 = t1 < 0 ? 0.0 : Math.pow(t1, 4) * this.dot(Simplex.GRAD3D[gi1], [x1, y1]);
        const t2 = 0.5 - x2 * x2 - y2 * y2;
        const n2 = t2 < 0 ? 0.0 : Math.pow(t2, 4) * this.dot(Simplex.GRAD3D[gi2], [x2, y2]);
    ...
    raw3D(x: number, y: number, z: number): number {
        ...
        const x1 = x0 - i1 + Simplex.G3; // Offsets for second corner in (x,y,z) coords
        const y1 = y0 - j1 + Simplex.G3;
        const z1 = z0 - k1 + Simplex.G3;
        const x2 = x0 - i2 + 2.0 * Simplex.G3; // Offsets for third corner in (x,y,z) coords
        const y2 = y0 - j2 + 2.0 * Simplex.G3;
        const z2 = z0 - k2 + 2.0 * Simplex.G3;
        const x3 = x0 - 1.0 + 3.0 * Simplex.G3; // Offsets for last corner in (x,y,z) coords
        ...
        const n0 = t0 < 0 ? 0.0 : Math.pow(t0, 4) * this.dot(Simplex.GRAD3D[gi0], [x0, y0, z0]);
        const t1 = 0.5 - x1 * x1 - y1 * y1 - z1 * z1;
        const n1 = t1 < 0 ? 0.0 : Math.pow(t1, 4) * this.dot(Simplex.GRAD3D[gi1], [x1, y1, z1]);
        const t2 = 0.5 - x2 * x2 - y2 * y2 - z2 * z2;
        const n2 = t2 < 0 ? 0.0 : Math.pow(t2, 4) * this.dot(Simplex.GRAD3D[gi2], [x2, y2, z2]);
        const t3 = 0.5 - x3 * x3 - y3 * y3 - z3 * z3;
        const n3 = t3 < 0 ? 0.0 : Math.pow(t3, 4) * this.dot(Simplex.GRAD3D[gi3], [x3, y3, z3]);
    ...
    raw4D(x: number, y: number, z: number, w: number): number {
        ...
        const t = (i + j + k + l) * Simplex.G4; // Factor for 4D unskewing
        ...
        const x1 = x0 - i1 + Simplex.G4; // Offsets for second corner in (x,y,z,w) coords
        const y1 = y0 - j1 + Simplex.G4;
        const z1 = z0 - k1 + Simplex.G4;
        const w1 = w0 - l1 + Simplex.G4;
        const x2 = x0 - i2 + 2.0 * Simplex.G4; // Offsets for third corner in (x,y,z,w) coords
        const y2 = y0 - j2 + 2.0 * Simplex.G4;
        const z2 = z0 - k2 + 2.0 * Simplex.G4;
        const w2 = w0 - l2 + 2.0 * Simplex.G4;
        const x3 = x0 - i3 + 3.0 * Simplex.G4; // Offsets for fourth corner in (x,y,z,w) coords
        const y3 = y0 - j3 + 3.0 * Simplex.G4;
        const z3 = z0 - k3 + 3.0 * Simplex.G4;
        const w3 = w0 - l3 + 3.0 * Simplex.G4;
        const x4 = x0 - 1.0 + 4.0 * Simplex.G4; // Offsets for last corner in (x,y,z,w) coords
        ...
        const n0 = t0 < 0 ? 0.0 : Math.pow(t0, 4) * this.dot(Simplex.GRAD4D[gi0], [x0, y0, z0, w0]);
        const t1 = 0.5 - x1 * x1 - y1 * y1 - z1 * z1 - w1 * w1;
        const n1 = t1 < 0 ? 0.0 : Math.pow(t1, 4) * this.dot(Simplex.GRAD4D[gi1], [x1, y1, z1, w1]);
        const t2 = 0.5 - x2 * x2 - y2 * y2 - z2 * z2 - w2 * w2;
        const n2 = t2 < 0 ? 0.0 : Math.pow(t2, 4) * this.dot(Simplex.GRAD4D[gi2], [x2, y2, z2, w2]);
        const t3 = 0.5 - x3 * x3 - y3 * y3 - z3 * z3 - w3 * w3;
        const n3 = t3 < 0 ? 0.0 : Math.pow(t3, 4) * this.dot(Simplex.GRAD4D[gi3], [x3, y3, z3, w3]);
        const t4 = 0.5 - x4 * x4 - y4 * y4 - z4 * z4 - w4 * w4;
        const n4 = t4 < 0 ? 0.0 : Math.pow(t4, 4) * this.dot(Simplex.GRAD4D[gi4], [x4, y4, z4, w4]);
